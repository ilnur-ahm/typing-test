...
export const compareChars: CompareCharsType = (
  ...
  const resultArr = charsArray.map((item, index) => {
    if (index === currentIndex && item.char === pressedKey) {
      // Увеличение индекса при правильном нажатии
      newCurrentIndex += 1;
      return {
        ...item,
        class: "right-char",
      ...
      // Увеличение количества ошибок при неправильном нажатии
      newMistakes += 1;
      return {
        ...item,
        class: "wrong-char",
  ...
  // Возвращение массива символов, текущего индекса и количества ошибок
